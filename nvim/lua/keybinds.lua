...
au("BufEnter", {
	...
	callback = function()
		vim.schedule(function()
			map(
				"<f4>",
				":!echo<space>\"require(rmarkdown);<space>render('<c-r>%')\"<space>|<space>R<space>--vanilla<enter>",
				{ silent = true }
			)
		end)
	end,
...
au("BufEnter", {
	...
	callback = function()
		vim.schedule(function()
			map("<f4>", ":!pandoc % -o %.pdf<CR>", { silent = true })
		end)
	end,
})
...
nmap("<leader>of", ":Explore<CR>", { silent = true, noremap = true })
